<script lang="ts">
  ...
  import { init as initViz, destroy as destroyViz } from '../visualizations/viz-toy8';
...
<div class="toy">
  <header>
    ...
    <section class="toy-description">
      <p>
        This toy models the scalar recurrence <code>x[t+1] = x[t] + d + n * r[t]</code>
        where <strong>d</strong> is a constant baseline drift and <strong>n</strong> is the
        noise amplitude multiplying a uniform sample r[t] ∈ [−1, 1]. Adjust the sliders
        to observe how noise amplitude affects trajectory variability.
      </p>
    </section>
    ...
    <div class="controls">
      <label>
        noise_level (n)
        <input type="range" min="0" max="1" step="0.01" bind:value={params.noise_level} />
        <span class="val">{params.noise_level.toFixed(2)}</span>
      </label>

      <label>
        baseline_drift (d)
        <input type="range" min="-1" max="1" step="0.01" bind:value={params.baseline_drift} />
