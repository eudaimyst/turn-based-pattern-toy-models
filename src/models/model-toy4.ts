...
export type Params = {
  u: number; // external scalar input in [-1,1]
  w_up: number; // weight for upward transitions in [0,1]
  w_down: number; // weight for downward transitions in [0,1]
};
...
export function initState(x = 0): State {
  return { x };
}
