...
type TrajPoint = { x: number; u: number };
...
const viz: {
  ...
  linesGroup: d3.Selection<SVGGElement, unknown, null, undefined> | null;
} = {
  svg: null,
  width: 600,
  height: 240,
...
export function init(container: HTMLElement, width = 600, height = 240) {
  viz.width = width;
  viz.height = height;
  container.innerHTML = '';
  ...
  return {
    render: (trajectories: Array<TrajPoint[]>) => render(trajectories),
...
function render(trajectories: Array<TrajPoint[]>) {
  if (!viz.svg || !viz.xScale || !viz.yScale || !viz.linesGroup) return;
  ...
  const paths = viz.linesGroup.selectAll<SVGPathElement, TrajPoint[]>('path').data(trajectories as TrajPoint[][]);

  paths.join(
    (enter) => enter.append('path').attr('fill', 'none').attr('stroke', (d, i) => colors[i % colors.length]).attr('stroke-width', 1.5).attr('d', (d) => line(d as any) ?? ''),
    (update) => update.attr('d', (d) => line(d as any) ?? ''),
    (exit) => exit.remove()
  );

  // endpoints
  const ends = viz.linesGroup.selectAll<SVGCircleElement, TrajPoint[]>('circle.end').data(trajectories as TrajPoint[][]);
  ...
  ends.join(
    (enter) =>
      enter
        ...
        .attr('cx', (d) => (d.length ? viz.xScale?.(d[d.length - 1].x) ?? 0 : 0))
        .attr('cy', (d) => (d.length ? viz.yScale?.(d[d.length - 1].u) ?? 0 : 0)),
    (update) => update.attr('cx', (d) => (d.length ? viz.xScale?.(d[d.length - 1].x) ?? 0 : 0)).attr('cy', (d) => (d.length ? viz.yScale?.(d[d.length - 1].u) ?? 0 : 0)),
    (exit) => exit.remove()
  );
}
