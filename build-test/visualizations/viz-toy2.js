...
function render(history) {
    ...
    pts.join((enter) => enter.append('circle').attr('r', 3).attr('fill', '#2d3748').attr('cx', (d) => (viz.xScale ? viz.xScale(d.x) : 0)).attr('cy', (d) => (viz.yScale ? viz.yScale(d.u) : 0)), (update) => update.attr('cx', (d) => (viz.xScale ? viz.xScale(d.x) : 0)).attr('cy', (d) => (viz.yScale ? viz.yScale(d.u) : 0)), (exit) => exit.remove());
}
export function destroy(container) {
    container.innerHTML = '';
    viz.svg = null;
}
//# sourceMappingURL=viz-toy2.js.map